...
class SettingsViewControllersContainer: UIViewController {
    ...
    fileprivate func configureContainedViewControllersData() {
        ...
        if userDataController.createProfileContainerView.profileImageView.image == nil || lastProfileURL != Auth.auth().currentUser?.photoURL {
            userDataController.createProfileContainerView.profileImageView.sd_setImage(with: Auth.auth().currentUser?.photoURL, placeholderImage: nil, options: [.progressiveLoad, .highPriority, .continueInBackground], completed: { (image, error, cacheType, url) in
    ...
    fileprivate func configureContainedViewControllers() {
        ...
        addChild(userDataController)
        addChild(accountSettingsController)
        ...
        userDataController.didMove(toParent: self)
        accountSettingsController.didMove(toParent: self)
    }
