...
class StorageTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cachedSize = SDImageCache.shared.config.diskCacheExpireType
        
        let cachedSizeInMegabyes = (Double(cachedSize.rawValue) * 0.000001).round(to: 1)
        
        print(cachedSize, cachedSizeInMegabyes)
        
        cell.accessoryType = .disclosureIndicator
        
        if Int(cachedSize.rawValue) > 0 {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let oversizeAlert = UIAlertController(title: "", message: "", preferredStyle: UIAlertController.Style.actionSheet)
